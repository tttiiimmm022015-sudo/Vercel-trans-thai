...
class GeminiServiceTests(unittest.TestCase):
    ...
    def test_available_keys_include_every_key(self) -> None:
        indexes = gemini_service._get_available_key_indexes(5, "model")
        self.assertEqual(sorted(indexes), [0, 1, 2, 3, 4])

    def test_cooldown_key_is_skipped(self) -> None:
        gemini_service._mark_key_cooldown("model", 2, 60)
        indexes = gemini_service._get_available_key_indexes(5, "model")
    ...
    def test_empty_primary_uses_fallback(self) -> None:
        ...
        with (
            patch.object(gemini_service, "get_settings", return_value=settings),
            patch.object(
                gemini_service,
                "_generate_with_model",
                side_effect=[
                    gemini_service.EmptyGeminiResponseError("empty"),
                    "翻譯成功",
                ],
            ) as generate,
        ):
            result = gemini_service.generate_translation("prompt")
