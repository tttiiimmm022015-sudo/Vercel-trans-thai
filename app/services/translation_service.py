...
def _translation_was_not_applied(
    ...
    if direction == "TH→ZH-TW":
        contains_source_language = bool(THAI_PATTERN.search(original))
    elif direction == "ZH-TW→TH":
        contains_source_language = bool(CHINESE_PATTERN.search(original))
    else:
        return False
...
def translate(
    ...
    try:
        ...
        logger.info(
            (
                "翻譯結果檢查：direction=%s input_length=%d "
                "output_length=%d same_as_input=%s"
            ),
            direction,
            len(cleaned_text),
            len(translated_text),
        ...
        if same_as_input:
            ...
            if not retry_result:
                logger.warning("Gemini 重試後回傳空白結果")
                return TRANSLATION_FAILED_MESSAGE
